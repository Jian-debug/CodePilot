...
const zh: Record<TranslationKey, string> = {
  ...
  'settings.workflowTitle': '动态工作流',
  'settings.workflowAutoSuggestTitle': '建议使用工作流',
  'settings.workflowAutoSuggestDesc': '对于大型、多步骤任务，提示助手使用工作流工具（规划步骤、逐步校验、失败时换更强模型重试）。最终仍由助手决定是否使用。',
  'settings.workflowConcurrencyTitle': '最大并行步骤数',
  'settings.workflowConcurrencyDesc': '可同时运行的相互独立的工作流步骤数量。有依赖关系的步骤始终会等待。设为 1 则串行执行。',
  'settings.workflowConcurrencySerial': '1（串行）',
  'settings.workflowRetriesTitle': '步骤重试次数',
  'settings.workflowRetriesDesc': '当某步骤用尽模型梯子后，在用最强模型（带校验反馈）额外重试多少次后才判失败。',
  'settings.workflowRetriesNone': '0（关闭）',
  'settings.workflowMaxDepthTitle': '拆解困难步骤',
  'settings.workflowMaxDepthDesc': '当某个困难步骤连最强模型都无法完成时，将其拆解为子工作流。此为最大嵌套深度（0 表示关闭）。',
  'settings.workflowMaxDepthOff': '0（关闭）',
